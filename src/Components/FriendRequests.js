...
const FriendRequests = () => {
  ...
  useEffect(() => {
    console.log(userid);
    axios
      .post("http://localhost:5001/friendrequestpage", {
        userid: userid,
      })
      .then((res) => {
  ...
  const delRequestHandler = (e) => {
    ...
    axios
      .post("http://localhost:5001/deleterequest", {
        requestid: requestid,
      })
      .then((res) => {
        if (res.status === 200) {
          ...
          swal(
            "Request rejected",
            "You have rejected this user's friend request",
            "info"
          );
        }
      });
  ...
  const acceptRequestHandler = (e) => {
    ...
    axios
      .post("http://localhost:5001/addfriend", {
        userid: userid,
        friendid: friendid,
      })
      .then((res) => {
  ...
  useEffect(() => {
    axios
      .post("http://localhost:5001/addfriendnotif", {
        userid: userid,
        friendid: friendidd,
      })
      .then((res) => {
    ...
    axios
      .post("http://localhost:5001/addfriendfeed", {
        userid: userid,
        friendid: friendidd,
      })
      .then((res) => {
                      ...
                      <div className="row">
                        ...
                        <div className="col-9 d-flex align-items-center friendsDetail">
                          <h1>
                            ...
                            <button>
                              <i
                                className="bi bi-check-lg green"
                                onClick={acceptRequestHandler}
                                id={profile.requestorid}
                              ></i>
                            </button>
                            <button>
                              <i
                                className="bi bi-x-lg red"
                                onClick={delRequestHandler}
