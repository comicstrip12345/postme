...
const RegisterProfile = () => {
    ...
    const autoregistr = (e) =>{
        ...
        if(cityValidity===true && birthdayValidity===true && photoValidity===true){
                ...
                <form onSubmit={autoregistr} encType="multipart/form-data">
                    <div className='row'>
                        <div className="col-9 form-floating mb-3 form">
                            <input type="text" className="form-control" style={{borderColor:`${errorBorderCityColor}`}} name="city" placeholder="city" onChange={handleCityInput}/>
                            <label htmlFor="fullname">City</label>
                        </div>
                        <div className='col-3 mb-3 errorMessage'>
                        {errorCityDisp && <p className='cityError'><i className="bi bi-exclamation-triangle-fill"></i> Input City</p>}
                        </div>
                    </div>
                    <div className='row'>
                        <div className="col-9 form-floating mb-3 form">
                            <input type="date" className="form-control" style={{borderColor:`${errorBorderBirthdayColor}`}} name="birthday"  placeholder="date" onChange={handleBirthdayInput}/>
                            <input type="text" className="form-control" name="username"  hidden value={username} readOnly/>
                            <label htmlFor="birthday">Birthday</label>
                        </div>
                        <div className='col-3 mb-3 errorMessage'>
                        {errorBirthdayDisp && <p className='birthdayError'><i className="bi bi-exclamation-triangle-fill"></i> Input Birthday</p>}
                        </div>
                    </div>
                    <div className='row'>
                        ...
                        <div className='col-3 mb-3 errorMessage'>
                        {errorPhotoDisp && <p className='photoError'><i className="bi bi-exclamation-triangle-fill"></i> Upload Photo</p>}
                        </div>
                    </div>
