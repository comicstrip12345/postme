...
const OtherProfile = () => {
  ...
  useEffect(() => {
    axios
      .post("http://localhost:5001/profile", { userid: wallOwnerId })
      .then((res) => {
        if (res.status === 200) {
    ...
    axios
      .post("http://localhost:5001/frsearcher", {
        userid: userid,
        wallid: wallOwnerId,
      })
      .then((res) => {
    ...
    axios
      .post("http://localhost:5001/flsearcher", {
        userid: userid,
        wallid: wallOwnerId,
      })
      .then((res) => {
  ...
  const addFriend = () => {
    axios
      .post("http://localhost:5001/friendrequest", {
        userid: userid,
        otherid: wallOwnerId,
      })
      .then((res) => {
  ...
  const deleteFriend = () => {
    axios
      .post("http://localhost:5001/deletefriend", {
        userid: userid,
        friendid: wallOwnerId,
      })
      .then((res) => {
        console.log(res);
        swal(
          "Deleted friend",
          "We have removed this user from your friend's list",
          "info"
        );
        setStateChanger(stateChanger + 1);
      });
              ...
              <div className="col-12 profileFeed">
                <div className="row">
                  ...
                  <div className="col-7 newsFeed">
                    <div className="row">
                      <OtherProfileFeedSettings
                        userid={`${userid}`}
                        wallOwnerId={`${wallOwnerId}`}
                        picpath={yourProfile.picpath}
                        ownerpicpath={profile.picpath}
                      />
                    </div>
                  </div>
                </div>
